...
public class UsuarioListaRegalosService {
    ...
    private static final String LISTA_NO_EXISTE = "Lista de regalos no encontrada con id ";
    ...
    public ListaRegalosEntity addInvitado(Long listaId, Long usuarioId) {
        ...
        ListaRegalosEntity lista = listaRegalosRepository.findById(listaId)
                .orElseThrow(() -> new EntityNotFoundException(LISTA_NO_EXISTE + listaId));
        ...
        if (usuario.getId().equals(lista.getCreador().getId())) {
            throw new IllegalArgumentException(
                    "El invitado no puede ser el mismo usuario que el creador de la lista.");
        }

        // Regla 2: no se pueden duplicar invitados
        boolean yaInvitado = lista.getInvitados().stream()
    ...
    @Transactional
    public ListaRegalosEntity removeInvitado(Long listaId, Long usuarioId) {
        ...
        boolean invitadoEncontrado = lista.getInvitados().removeIf(u -> u.getId().equals(usuarioId));
    ...
    public boolean isUsuarioInvitado(Long listaId, Long usuarioId) {

        ListaRegalosEntity lista = listaRegalosRepository.findById(listaId)
                .orElseThrow(() -> new EntityNotFoundException(LISTA_NO_EXISTE + listaId));

        return lista.getInvitados().stream().anyMatch(u -> u.getId().equals(usuarioId));
    }
}
